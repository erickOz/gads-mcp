# Conectar este MCP a un cliente

Cubre Claude Desktop, Claude Code, Codex y OpenCode.

---

## ⚠️ Antes que nada: dónde puede vivir este repo

**Nunca en una carpeta sincronizada a la nube** (Google Drive File Stream, iCloud
Drive, OneDrive, Dropbox). Los archivos quedan *online-only* y cada lectura es una
descarga: el arranque del servidor pasa de segundos a minutos y **todo cliente MCP
corta el handshake por timeout** ("Connection closed"). Además esos sistemas rompen
los symlinks del `.venv` y borran el bit de ejecución de los `.sh`.

Ruta actual del repo: **`/Users/erickoz/Developer/gads-mcp`** (disco local).

## Dónde están registradas las rutas (cambian todas juntas)

Si mueves el repo, hay que actualizar estos cinco archivos **y** recrear el `.venv`
con `uv sync` (el editable install guarda rutas absolutas):

| Cliente | Archivo de configuración |
|---|---|
| Claude Code | `~/.claude.json` → `mcpServers.google-ads-mcp` |
| Claude Code | `~/.claude/settings.json` → `mcpServers.google-ads-mcp` |
...
```jsonc
// Claude Desktop / OpenCode — invocan el intérprete del venv directamente
"command": "/Users/erickoz/Developer/gads-mcp/.venv/bin/python",
"args": ["-m", "ads_mcp.stdio"],
"env": { "GOOGLE_ADS_CREDENTIALS": "/Users/erickoz/.config/google-ads.yaml" }

// Claude Code / Codex — vía uv, que recrea el venv solo si falta
"command": "/Users/erickoz/.local/bin/uv",
"args": ["run", "--directory", "/Users/erickoz/Developer/gads-mcp", "-m", "ads_mcp.stdio"],
"env": { "GOOGLE_ADS_CREDENTIALS": "/Users/erickoz/.config/google-ads.yaml" }
```

Comprobación rápida de que el servidor arranca (debe quedarse esperando en stdin,
sin errores, en pocos segundos):
  ...
  "mcpServers": {
    "google-ads": {
      ...
      "args": [
        "run",
        "--directory",
        "/Users/erickoz/Developer/gads-mcp",
        "-m",
        "ads_mcp.stdio"
      ],
      "env": {
        "GOOGLE_ADS_CREDENTIALS": "/Users/erickoz/.config/google-ads.yaml"
      }
    }
  }
}
...
Set `GOOGLE_ADS_CREDENTIALS=/Users/erickoz/.config/google-ads.yaml`.
